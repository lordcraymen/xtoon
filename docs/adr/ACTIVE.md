...
| ADR-0009 | ADR-0009: Two-Phase Validation Strategy | Accepted | Establishes a two-phase validation model (parse-time structural validation, expansion-time content validation) that applies to all column modifiers, ensuring fail-fast error detection while deferring content-specific validation to expansion. |
